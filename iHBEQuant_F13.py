...
def write_summary(path,records,atoms,monomers,mono_e,cluster_e,cfg,xyz,mode,run_time,prefix,out_dir):
    ...
    # HB table
    W   = [5,18,10,10,10,10,11,15,15,15]
    SEP = "+"+"+".join("-"*w for w in W)+"+"
    HDR = (f"|{'HB#':^{W[0]}}|{'D-H...A':^{W[1]}}|{'Connection':^{W[2]}}|{'D-H(A)':^{W[3]}}"
           f"|{'H...A(A)':^{W[4]}}|{'A...D(A)':^{W[5]}}|{'Angle(deg)':^{W[6]}}"
           f"|{'MTA_HBE':^{W[7]}}|{'Dimer':^{W[8]}}|{'Coop':^{W[9]}}|")
    lines += [SEP,HDR,SEP]
    for r in records:
        hb,res = r["hb"],r["res"]
        label  = f"{lbl(hb['di'],atoms)}-{lbl(hb['hi'],atoms)}...{lbl(hb['ai'],atoms)}"
        conn   = f"M{hb['dm']+1}_M{hb['am']+1}"
        lines.append(f"|{r['n']:^{W[0]}}|{label:^{W[1]}}|{conn:^{W[2]}}|{fmt(hb['dh_dist']):^{W[3]}}"
                     f"|{fmt(hb['ha_dist']):^{W[4]}}|{fmt(hb['da_dist']):^{W[5]}}|{fmt(hb['angle']):^{W[6]}}"
                     f"|{fmt(res['hbe']):^{W[7]}}|{fmt(res['dimer']):^{W[8]}}|{fmt(res['coop']):^{W[9]}}|")
    lines += [SEP,""]
